...
def initialize_db(conn) -> None:
    ...
    cursor.execute("""CREATE TABLE IF NOT EXISTS card_decks (
                        ...
                        next_review_date TEXT DEFAULT "Complete Review First",
...
def create_card(conn, deck_id: int, question: str, answer: str) -> None:
    ...
    next_review = datetime(2024, 1, 1, 0, 0, 0, 0)
...
def update_cards_next_review_date(conn, card_id: int, new_sm2_data: tuple[int, float, int]):
    cursor = conn.cursor()
    
    new_repetition, new_ef, new_interval = new_sm2_data
    now = datetime.now()
    timestamp = now.isoformat()
    next_review_time = (now + timedelta(days=new_interval)).replace(hour=0, minute=0, second=0, microsecond=0).isoformat()
...
#updates a decks next review date
def update_deck_next_review_date(conn, new_interval, deck_id):
    cursor = conn.cursor()

    now = datetime.now()
    timestamp = now.isoformat()
    next_review_time = (now + timedelta(days=new_interval)).strftime("%m/%d/%y")

    print(next_review_time)
    sql = "UPDATE card_decks SET next_review_date = ?, updated_at = ? WHERE id = ?"
    data = (next_review_time, timestamp, deck_id)
    cursor.execute(sql, data)
...
#updates a decks next review value
def update_decks_next_review(conn):
    cursor = conn.cursor()
